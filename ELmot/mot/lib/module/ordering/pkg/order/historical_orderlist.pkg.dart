...
import 'package:online_trading/module/ordering/model/order/historical_order_list.model.dart';

RxList<HistoricalOrderList> listHistorical = <HistoricalOrderList>[].obs;
HistoricalOrderList historicalorderListReq(Uint8List buf) {
  var getController = Get.find<EncryptControll>();
  getController.readPos.value = Constans.PACKAGE_HEADER_LENGTH;

  HistoricalOrderList datas = HistoricalOrderList();
  ...
  for (var i = 0; i < arrayL; i++) {
    ArrayHistoricalOrderList dataAr = ArrayHistoricalOrderList();
    ...
    if (dataAr.orderStatus == 0) {
      int rjL = getController.encrypt2(buf);
      dataAr.rejectNote = getController.getAsciiString(buf, rjL);
    }
